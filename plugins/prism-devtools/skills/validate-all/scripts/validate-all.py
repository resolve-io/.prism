...
Checks:
  ...
  4. Plugin wiring validation (validate-plugin.py) — structural component wiring
...
def main():
    ...
    scripts = {
        ...
        'validate-plugin.py': find_script(plugin_dir, 'validate-plugin.py'),
    }
    ...
    print("[1/4] Documentation validation (6-phase structural scan)")
    ...
    print("[2/4] Link validation (broken reference check)")
    r = run_check('Links', [
    ...
    print("[3/4] Portability check (PC001-PC005)")
    ...
    # --- Check 4: Plugin wiring validation ---
    print("[4/4] Plugin wiring validation (6-phase structural check)")
    r = run_check('PluginWiring', [
        sys.executable, str(scripts['validate-plugin.py']),
        '--root', str(scan_root), '--json'
    ])
    results.append(r)
    data = parse_json_output(r['stdout'])
    if r['exit_code'] == 1 and data:
        errors = data.get('summary', {}).get('errors', '?')
        print(f"  FAIL: {errors} wiring error(s)")
        blocked = True
    elif r['exit_code'] == 1:
        print("  FAIL: Plugin wiring errors found")
        blocked = True
    elif r['exit_code'] == 2:
        print(f"  SKIP: Script error — {r['stderr'][:100]}")
    elif data and data.get('summary', {}).get('warnings', 0):
    ...
    if blocked:
        ...
        print(f"  python {scripts['validate-plugin.py'].relative_to(plugin_dir)}")
        sys.exit(1)
    else:
