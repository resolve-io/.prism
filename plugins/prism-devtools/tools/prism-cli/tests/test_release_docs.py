...
class TestAC4_PluginJsonVersion:
    ...
    def test_ac4_plugin_json_version_is_371(self):
        ...
        Requirement: Version bump from 3.9.3 → 3.10.0 for prism-bug platform & hook diagnostics
        Expected: json.loads(plugin.json)['version'] == '3.10.0'
        """
        data = json.loads(PLUGIN_JSON.read_text(encoding="utf-8"))
        assert data.get("version") == "3.10.0", (
            f"plugin.json version is '{data.get('version')}', expected '3.10.0'"
        )
...
class TestAC5_VersionOrdering:
    ...
    def test_ac5_250_is_latest_version(self):
        ...
        assert first_version.group(1) == "3.10.0", (
            f"First version in CHANGELOG is '{first_version.group(1)}', "
            f"expected '3.10.0' (should be latest)"
        )
