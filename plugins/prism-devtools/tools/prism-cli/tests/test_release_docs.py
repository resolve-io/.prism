...
AC-4: plugin.json version is "3.0.3"
...
# ---------------------------------------------------------------------------
# AC-4: plugin.json version is 3.0.2
# ---------------------------------------------------------------------------
class TestAC4_PluginJsonVersion:
    ...
    def test_ac4_plugin_json_version_is_303(self):
        """
        AC-4: plugin.json 'version' field is '3.0.3'
        Requirement: Version bump from 3.0.2 → 3.0.3 for Brain CLI entrypoint (prism-a080)
        Expected: json.loads(plugin.json)['version'] == '3.0.3'
        """
        data = json.loads(PLUGIN_JSON.read_text(encoding="utf-8"))
        assert data.get("version") == "3.0.3", (
            f"plugin.json version is '{data.get('version')}', expected '3.0.3'"
        )
