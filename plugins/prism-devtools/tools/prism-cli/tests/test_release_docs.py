...
class TestAC4_PluginJsonVersion:
    ...
    def test_ac4_plugin_json_version_is_340(self):
        """
        AC-4: plugin.json 'version' field is '3.4.0'
        Requirement: Version bump from 3.3.0 → 3.4.0 for behavior fixes batch (dotnet runner path fix, brain/conductor diagnostics, skill discovery diagnostics, brain test coverage)
        Expected: json.loads(plugin.json)['version'] == '3.4.0'
        """
        data = json.loads(PLUGIN_JSON.read_text(encoding="utf-8"))
        assert data.get("version") == "3.4.0", (
            f"plugin.json version is '{data.get('version')}', expected '3.4.0'"
        )
